...
put admin coupon distribute settings by setting id Fail With Wrong Url
   ...
   ${essential_params}  create list  enabled=False  coupon_pool_id=${coupon_pool_id}  
   ${unessential_params}  create list  
   run every case by params  put admin coupon distribute settings by setting id Fail 404  ${essential_params}  ${unessential_params}    setting_id=${w}
...
delete admin coupon distribute settings by setting id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   delete admin coupon distribute settings by setting id Fail 404    setting_id=${w}
