...
patch admin coupon point records by coupon point record id Fail With Wrong Url
   ...
   ${essential_params}  create list  
   ${unessential_params}  create list  point_remark=${point_remark}  
   run every case by params  patch admin coupon point records by coupon point record id Fail 404  ${essential_params}  ${unessential_params}    coupon_point_record_id=${w}
...
delete admin coupon point records by coupon point record id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   delete admin coupon point records by coupon point record id Fail 404    coupon_point_record_id=${w}
