...
post wechat userinfo decrypt Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create list  nickName=${nickName_422}  avatarUrl=${avatarUrl_422}  code=${code_422}  rawData=${rawData_422}  signature=${signature_422}  encryptedData=${encryptedData_422}  iv=${iv_422}  
   ${unessential_params}  create list  
...
post login by unionid Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create list  client_id=${client_id_422}  secret=${secret_422}  unionid=${unionid_422}  
   ${unessential_params}  create list  
...
post login by openid Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create list  client_id=${client_id_422}  secret=${secret_422}  openid=${openid_422}  
   ${unessential_params}  create list  
...
post login by mobile Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create list  client_id=${client_id_422}  secret=${secret_422}  mobile=${mobile_422}  serie=${serie_422}  captcha=${captcha_422}  
   ${unessential_params}  create list  openid=${openid_422}  union_id=${union_id_422}  mn_open_id=${mn_open_id_422}  
...
get openid by tokenid Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create list  tokenid=${tokenid_422}  
   ${unessential_params}  create list  
