...
post orders pay by order id Fail With Wrong Url
   ...
   run every case by params  post orders pay by order id Fail 404  ${essential_params}  ${unessential_params}    order_id/pay=${w}  order_id=${r}
...
post orders pay by order id Success 
   ...
   ${essential_params}  create list  pay_type=${pay_type}  
   ${unessential_params}  create list  
   run every case by params  post orders pay by order id Success 200  ${essential_params}  ${unessential_params}    order_id/pay=${order_id/pay}  order_id=${order_id}
...
post orders pay by order id Fail With Wrong Params
   ...
   ${unessential_params}  create list  
   run every case by params  post orders pay by order id Fail 422  ${essential_params}  ${unessential_params}    order_id/pay=${order_id/pay}  order_id=${order_id}  success=False
...
get orders by order id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   get orders by order id Fail 404    order_id/pay=${w}  order_id=${r}
...
get orders by order id Success 
   ...
   [Tags]           Respcode:200
   get orders by order id Success 200    order_id/pay=${order_id/pay}  order_id=${order_id}
