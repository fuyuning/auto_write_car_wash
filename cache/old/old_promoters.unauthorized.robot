...
put admin promoters by promoter id Fail Without Login
   ...
   ${essential_params}  create list  
   ${unessential_params}  create list  name=${name}  mobile=${mobile}  promoter_remark=${promoter_remark}  enabled=False  
   run every case by params  put admin promoters by promoter id Fail 403  ${essential_params}  ${unessential_params}    promoter_id=${promoter_id}  promoter_id/enabled=${promoter_id/enabled}
...
patch admin promoters enabled by promoter id Fail Without Login
   ...
   run every case by params  patch admin promoters enabled by promoter id Fail 403  ${essential_params}  ${unessential_params}    promoter_id=${promoter_id}  promoter_id/enabled=${promoter_id/enabled}
...
delete admin promoters by promoter id Fail Without Login
   ...
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   delete admin promoters by promoter id Fail 403    promoter_id=${promoter_id}  promoter_id/enabled=${promoter_id/enabled}
