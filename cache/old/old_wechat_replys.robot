...
put admin wechat replys by wechat reply id Fail With Wrong Url
   ...
   ${essential_params}  create list  category=${category}  reply_type=${reply_type}  content=${content}  
   ${unessential_params}  create list  
   run every case by params  put admin wechat replys by wechat reply id Fail 404  ${essential_params}  ${unessential_params}    wechat_reply_id=${w}
...
delete admin wechat replys by wechat reply id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   delete admin wechat replys by wechat reply id Fail 404    wechat_reply_id=${w}
