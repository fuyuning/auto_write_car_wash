...
class AutoWriteRobot(object):
    ...
    def _parse_file(self):
        ...
        with open('%s/auto_write_robot.csv' % self.document_path, 'r') as file:
            ...
            # for row in reader:
            #     print(row['服务端'], row['模块名'], row['接口名'], row['请求方式'], row['url'], row['参数名'], row['参数类型'],
            #           row['是否必传'], row['响应码情况'], row['返回实体'])
            ...
            #         index2 += 1
            # all_model_list.append(model_list)
            # print('总计写入模块：' + str(len(all_model_list) - 1) + '个')
    ...
    def _write_library_head(self, lib_name, model_name):
        print('正在写入文件：' + lib_name + '/' + '__init__.py')
        ...
        print('正在写入文件：' + lib_name + '/' + class_name + 'Library.py')
    ...
    def _write_robot_file_settings(self, service_name, model_name):
        ...
        if folder:
            os.rename('../tests/' + full_name + '/' + model_name + 's.robot',
                      '../cache/old/old_' + model_name + 's.robot')
        ...
        print('正在写入文件：' + '../tests/' + full_name + '/' + model_name + 's.robot')
        ...
        print('正在写入文件：' + '../tests/' + full_name + '/' + model_name + 's.unauthorized.robot')
