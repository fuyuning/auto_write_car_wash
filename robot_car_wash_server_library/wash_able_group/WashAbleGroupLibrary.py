...
class WashAbleGroupLibrary(CommonLibrary):
    ...
    def put_admin_wash_able_groups_by_wash_able_group_id(self, wash_able_group_id, **kwargs):
        ...
        data = {}
        for k, v in kwargs.items():
            if k in ("name", "group_time_range", "expect_time", "expect_day", "is_diff_car_type", "people_num", "people_day_num", "service_groups", "status", "start_time", "end_time", "day_in_week", "car_washers", "spec_people_day_num", "time_loop_set", "auto_group"):
                data[k] = v
