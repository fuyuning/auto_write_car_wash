...
class WashRecordLibrary(CommonLibrary):
    ...
    def patch_admin_wash_records_serve_time_by_wash_record_id(self, wash_record_id, **kwargs):
        url = "{SERVER_DOMAIN}/admin/wash_records/{wash_record_id}/serve_time".format(
            SERVER_DOMAIN=self.SERVER_DOMAIN, wash_record_id=wash_record_id)
        data = {}
        for k, v in kwargs.items():
            if k in ("serve_time", "expect_time"):
                data[k] = v
        return self.client.patch(url, json=data)

    def patch_admin_wash_records_position_by_wash_record_id(self, wash_record_id, **kwargs):
        url = "{SERVER_DOMAIN}/admin/wash_records/{wash_record_id}/position".format(
            SERVER_DOMAIN=self.SERVER_DOMAIN, wash_record_id=wash_record_id)
        data = {}
        for k, v in kwargs.items():
            if k in ("position", ):
