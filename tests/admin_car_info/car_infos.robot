...
get admin car infos Success 
   ...
   ${unessential_params}  create dictionary  car_id=${car_id}  car_type=${car_type}  page_num=${page_num}  page_size=${page_size}  
...
get admin car infos Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  
   ${unessential_params}  create dictionary  car_id=${car_id_422}  car_type=${car_type_422}  page_num=${page_num_422}  page_size=${page_size_422}  
...
put admin car infos by car info id Success 
   ...
   ${unessential_params}  create dictionary  car_model=${car_model}  car_brand=${car_brand}  car_series=${car_series}  car_color=${car_color}  car_frame_no=${car_frame_no}  licensed_time=${licensed_time}  car_year=${car_year}  car_images=${car_images}  car_tire_pressure=${car_tire_pressure}  description=${description}  tag=${tag}  
...
put admin car infos by car info id Fail With Wrong Params
   ...
   ${unessential_params}  create dictionary  car_model=${car_model_422}  car_brand=${car_brand_422}  car_series=${car_series_422}  car_color=${car_color_422}  car_frame_no=${car_frame_no_422}  licensed_time=${licensed_time_422}  car_year=${car_year_422}  car_images=${car_images_422}  car_tire_pressure=${car_tire_pressure_422}  description=${description_422}  tag=${tag_422}  
...
put admin car infos by car info id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin car infos by car info id fail 404     car_info_id=${wrong_url_id}  car_id=${car_id}    car_model=${car_model}  car_brand=${car_brand}  car_series=${car_series}  car_color=${car_color}  car_frame_no=${car_frame_no}  licensed_time=${licensed_time}  car_year=${car_year}  car_images=${car_images}  car_tire_pressure=${car_tire_pressure}  description=${description}  tag=${tag}  
