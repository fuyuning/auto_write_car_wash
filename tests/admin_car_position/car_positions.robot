...
post admin car positions manual Fail With Wrong Params
   ...
   ${unessential_params}  create dictionary  wash_area_id=${wash_area_id_422}  entry_time=${entry_time_422}  lon=${lon_422}  lat=${lat_422}  phase=${phase_422}  floor=${floor_422}  zone=${zone_422}  space_no=${space_no_422}  ownership=${ownership_422}  
...
post admin car positions manual Success 
   ...
   ${unessential_params}  create dictionary  wash_area_id=${wash_area_id}  entry_time=${entry_time}  lon=${lon}  lat=${lat}  phase=${phase}  floor=${floor}  zone=${zone}  space_no=${space_no}  ownership=${ownership}  
...
get admin car positions car postion id by  Fail With Wrong Url
   ...
   [Tags]           Respcode:404
   get admin car positions car postion id by  fail 404     =${wrong_url_id}
...
put admin car positions by car position id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin car positions by car position id fail 404     car_position_id=${wrong_url_id}  car_id=${car_id}  parking_id=${parking_id}    wash_area_id=${wash_area_id}  entry_time=${entry_time}  lon=${lon}  lat=${lat}  phase=${phase}  floor=${floor}  zone=${zone}  space_no=${space_no}  ownership=${ownership}  
