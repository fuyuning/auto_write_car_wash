...
post admin parkings Success 
   ...
   [Tags]           Respcode:201
   ${essential_params}  create dictionary  parking_name=${parking_name}  position=${position}  
   ${unessential_params}  create dictionary  alias_name=${alias_name}  wash_area_id=${wash_area_id}  out_parking_id=${out_parking_id}  enabled=False  lon=${lon}  lat=${lat}  parking_banners=${parking_banners}  use_home_banner=False  service_introduce=${service_introduce}  address=${address}  use_home_illustrate=False  province=${province}  city=${city}  district=${district}  landing_content=${Please_input}  parking_map=${parking_map}  
...
post admin parkings Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  parking_name=${parking_name_422}  position=${position_422}  
...
get admin parkings Success 
   ...
   [Tags]           Respcode:200
   ${essential_params}  create dictionary  
   ${unessential_params}  create dictionary  parking_name=${parking_name}  wash_area_id=${wash_area_id}  enabled=False  page_num=${page_num}  page_size=${page_size}  
...
get admin parkings Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  
   ${unessential_params}  create dictionary  parking_name=${parking_name_422}  wash_area_id=${wash_area_id_422}  enabled=ThisIsRobot!  page_num=${page_num_422}  page_size=${page_size_422}  
...
put admin parkings by parking id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin parkings by parking id fail 404     parking_id=${wrong_url_id}  parking_name=${parking_name}  position=${position}    alias_name=${alias_name}  wash_area_id=${wash_area_id}  out_parking_id=${out_parking_id}  enabled=False  lon=${lon}  lat=${lat}  parking_banners=${parking_banners}  use_home_banner=False  service_introduce=${service_introduce}  address=${address}  use_home_illustrate=False  province=${province}  city=${city}  district=${district}  landing_content=${Please_input}  parking_map=${parking_map}  
...
patch admin parkings enabled by parking id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   patch admin parkings enabled by parking id fail 404     parking_id=${wrong_url_id}  enabled=False    
