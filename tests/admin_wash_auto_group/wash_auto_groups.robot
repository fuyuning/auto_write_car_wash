...
put admin wash auto groups by wash auto group id Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  name=${name_422}  group_time_range=${group_time_range_422}  expect_time=${expect_time_422}  expect_day=${expect_day_422}  is_diff_car_type=ThisIsRobot!  people_num=${people_num_422}  people_day_num=${people_day_num_422}  service_groups=${service_groups_422}  status=${status_422}  start_time=${start_time_422}  end_time=${end_time_422}  
   ${unessential_params}  create dictionary  day_in_week=${day_in_week_422}  car_washers=${car_washers_422}  is_auto_robot=ThisIsRobot!  auto_group=ThisIsRobot!  subscribe=ThisIsRobot!  subscribe_name=${subscribe_name_422}  subscribe_day_num=${subscribe_day_num_422}  subscribe_expect_time=${subscribe_expect_time_422}  subscribe_expect_day=${subscribe_expect_day_422}  
...
put admin wash auto groups by wash auto group id Success 
   ...
   [Tags]           Respcode:201
   ${essential_params}  create dictionary  name=${name}  group_time_range=${group_time_range}  expect_time=${expect_time}  expect_day=${expect_day}  is_diff_car_type=False  people_num=${people_num}  people_day_num=${people_day_num}  service_groups=${service_groups}  status=${status}  start_time=${start_time}  end_time=${end_time}  
   ${unessential_params}  create dictionary  day_in_week=${day_in_week}  car_washers=${car_washers}  is_auto_robot=False  auto_group=False  subscribe=False  subscribe_name=${subscribe_name}  subscribe_day_num=${subscribe_day_num}  subscribe_expect_time=${subscribe_expect_time}  subscribe_expect_day=${subscribe_expect_day}  
...
put admin wash auto groups by wash auto group id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin wash auto groups by wash auto group id fail 404     wash_auto_group_id=${wrong_url_id}  name=${name}  group_time_range=${group_time_range}  expect_time=${expect_time}  expect_day=${expect_day}  is_diff_car_type=False  people_num=${people_num}  people_day_num=${people_day_num}  service_groups=${service_groups}  status=${status}  start_time=${start_time}  end_time=${end_time}    day_in_week=${day_in_week}  car_washers=${car_washers}  is_auto_robot=False  auto_group=False  subscribe=False  subscribe_name=${subscribe_name}  subscribe_day_num=${subscribe_day_num}  subscribe_expect_time=${subscribe_expect_time}  subscribe_expect_day=${subscribe_expect_day}  
...
patch admin wash auto group by wash auto group id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   patch admin wash auto group by wash auto group id fail 404     wash_auto_group_id=${wrong_url_id}  status=${status}    
