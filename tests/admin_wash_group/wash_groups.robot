...
post admin wash groups Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create list  name=${name_422}  valid_start_time=${Please_input}  valid_end_time=${Please_input}  is_diff_car_type=ThisIsRobot!  service_groups=${service_groups_422}  people_num=${people_num_422}  wash_area_id=${wash_area_id_422}  expect_time=${Please_input}  car_washer_ids=${car_washer_ids_422}  
...
post admin wash groups join by wash group id Success 204
   [Arguments]  &{kwargs}
   ${resp}=  post admin wash groups join by wash group id  &{kwargs}
