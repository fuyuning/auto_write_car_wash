...
patch admin wash records position by wash record id Fail With Wrong Params
   [Documentation]  接口名:修改车辆位置${\n}
   ...              请求方式:Patch${\n}
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  position=${position_422}  
   ${unessential_params}  create dictionary  
   run every case by params   patch admin wash records position by wash record id fail 422   ${essential_params}  ${unessential_params}    wash_record_id=${wash_record_id}  success=False

patch admin wash records position by wash record id Success 
   [Documentation]  接口名:修改车辆位置${\n}
   ...              请求方式:Patch${\n}
   ...              预期结果:输入正确参数,http响应码返回 204,无Json数据返回。
   [Tags]           Respcode:204
   ${essential_params}  create dictionary  position=${position}  
   ${unessential_params}  create dictionary  
   run every case by params   patch admin wash records position by wash record id success 204   ${essential_params}  ${unessential_params}    wash_record_id=${wash_record_id}

patch admin wash records position by wash record id Fail With Wrong Url
   [Documentation]  接口名:修改车辆位置${\n}
   ...              请求方式:Patch${\n}
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   patch admin wash records position by wash record id fail 404     wash_record_id=${wrong_url_id}  position=${position}    
...
patch admin wash records position by wash record id Fail 422
   [Arguments]  &{kwargs}
   ${resp}=  patch admin wash records position by wash record id  &{kwargs}
   expect status is 422  ${resp}  

patch admin wash records position by wash record id Success 204
   [Arguments]  &{kwargs}
   ${resp}=  patch admin wash records position by wash record id  &{kwargs}
   expect status is 204  ${resp}  

patch admin wash records position by wash record id Fail 404
   [Arguments]  &{kwargs}
   ${resp}=  patch admin wash records position by wash record id  &{kwargs}
   expect status is 404  ${resp}  
