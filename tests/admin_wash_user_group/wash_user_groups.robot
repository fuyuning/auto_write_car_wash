...
post admin wash user groups Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  name=${name_422}  order_by=${order_by_422}  
...
post admin wash user groups Success 
   ...
   [Tags]           Respcode:201
   ${essential_params}  create dictionary  name=${name}  order_by=${order_by}  
   ${unessential_params}  create dictionary  
...
get admin wash user groups by wash user group id Success 
   [Documentation]  接口名:分组信息获取${\n}
   ...              请求方式:Get${\n}
   ...              预期结果:输入正确参数,http响应码返回 200,返回的Json数据为 wash_user_group 对象。
   [Tags]           Respcode:200
   get admin wash user groups by wash user group id success 200     wash_user_group_id=${wash_user_group_id}
...
put admin wash user groups by wash user group id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin wash user groups by wash user group id fail 404     wash_user_group_id=${wrong_url_id}  name=${name}  order_by=${order_by}    
...
get admin wash user groups by wash user group id Success 200
   [Arguments]  &{kwargs}
   ${resp}=  get admin wash user groups by wash user group id  &{kwargs}
   expect status is 200  ${resp}  admin_wash_user_group/get_admin_wash_user_groups_by_wash_user_group_id_200.json
   ${wash_user_group_id}  set variable if  ${resp.json()}!=[]  ${resp.json()[0]['wash_user_group_id']}
   set global variable   ${wash_user_group_id}
