...
post admin wechat keywords Success 
   ...
   [Tags]           Respcode:201
   ${essential_params}  create dictionary  name=${name}  keyword=${keyword}  reply_type=${reply_type}  content=${content}  
...
post admin wechat keywords Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  name=${name_422}  keyword=${keyword_422}  reply_type=${reply_type_422}  content=${content_422}  
   ${unessential_params}  create dictionary  
...
put admin wechat keywords by wechat keyword id Fail With Wrong Url
   ...
   ...              预期结果:输入正确参数及错误的url,http响应码返回 404,无Json数据返回。
   [Tags]           Respcode:404
   put admin wechat keywords by wechat keyword id fail 404     wechat_keyword_id=${wrong_url_id}  name=${name}  keyword=${keyword}  reply_type=${reply_type}  content=${content}    
