...
post admin wechat keywords Fail Without Login
   ...
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   post admin wechat keywords fail 403   name=${name}  keyword=${keyword}  reply_type=${reply_type}  content=${content}    
...
put admin wechat keywords by wechat keyword id Fail Without Login
   ...
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   put admin wechat keywords by wechat keyword id fail 403     wechat_keyword_id=${wechat_keyword_id}  name=${name}  keyword=${keyword}  reply_type=${reply_type}  content=${content}    
...
*** Variables ***
${wechat_keyword_id}  1a2b3c4d5e6f7zz
