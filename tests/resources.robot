...
Status Should be
    ...
    [Teardown]          run keyword if  '${KEYWORD_STATUS}'=='FAIL'  log  ${SUITE_NAME}模块中的接口：${resp.url[53:]} 错误。test_case：${TEST_NAME}:应返回${status_code}，但返回${resp.status_code}。  WARN
...
To Validate Json
    ...
    [Teardown]          run keyword if  '${KEYWORD_STATUS}'=='FAIL'  Log  ${SUITE_NAME}模块中的接口：${resp.url[53:]}错误。test_case：${TEST_NAME}:Json验证错误。  ERROR
    [Return]            ${KEYWORD_STATUS}
