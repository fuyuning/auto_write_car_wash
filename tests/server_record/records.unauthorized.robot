...
get projects cameras spots parking records by project id camera id spot id Fail Without Login
   [Documentation]  接口名:获取停车记录列表${\n}
   ...              请求方式:Get${\n}
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   get projects cameras spots parking records by project id camera id spot id fail 403     project_id=${project_id}  camera_id=${camera_id}  spot_id=${spot_id}

get projects spot records by project id Fail Without Login
   [Documentation]  接口名:获取泊位记录列表${\n}
   ...              请求方式:Get${\n}
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   get projects spot records by project id fail 403     project_id=${project_id}

get projects cameras spot records by project id camera id spot record id Fail Without Login
   [Documentation]  接口名:获取泊位记录详情${\n}
   ...              请求方式:Get${\n}
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   get projects cameras spot records by project id camera id spot record id fail 403     project_id=${project_id}  camera_id=${camera_id}  spot_record_id=${spot_record_id}
...
${spot_record_id}  1a2b3c4d5e6f7zz
...
get projects cameras spots parking records by project id camera id spot id Fail 403
   [Arguments]  &{kwargs}
   ${resp}=  get projects cameras spots parking records by project id camera id spot id  &{kwargs}
   expect status is 403  ${resp}  

get projects spot records by project id Fail 403
   [Arguments]  &{kwargs}
   ${resp}=  get projects spot records by project id  &{kwargs}
   expect status is 403  ${resp}  

get projects cameras spot records by project id camera id spot record id Fail 403
   [Arguments]  &{kwargs}
