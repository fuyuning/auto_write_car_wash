...
post user lucky flop award Success 
   ...
   [Tags]           Respcode:200
   ${essential_params}  create dictionary  lucky_flop_award_id=${lucky_flop_award_id}  custom_price=${custom_price}  
...
post user lucky flop award Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  lucky_flop_award_id=${lucky_flop_award_id_422}  custom_price=${custom_price_422}  
   ${unessential_params}  create dictionary  
