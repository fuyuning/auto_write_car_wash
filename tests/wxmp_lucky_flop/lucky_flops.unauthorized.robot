...
post user lucky flop Fail Without Login
   ...
   [Tags]           Respcode:403
   post user lucky flop fail 403   wash_normal_id=${wash_normal_id}  parking_id=${parking_id}  service_group_id=${service_group_id}  car_series_id=${car_series_id}    
...
post user lucky flop award Fail Without Login
   ...
   ...              预期结果:未登录,http响应码返回 403,无Json数据返回。
   [Tags]           Respcode:403
   post user lucky flop award fail 403   lucky_flop_award_id=${lucky_flop_award_id}  wash_normal_id=${wash_normal_id}  lucky_flop_id=${lucky_flop_id}  custom_price=${custom_price}    
