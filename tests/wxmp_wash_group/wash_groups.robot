...
post user wash groups Success 
   ...
   [Tags]           Respcode:200
   ${essential_params}  create dictionary  wash_able_group_id=${wash_able_group_id}  parking_id=${parking_id}  
   ${unessential_params}  create dictionary  
   run every case by params   post user wash groups success 200   ${essential_params}  ${unessential_params}
...
post user wash groups Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  wash_able_group_id=${wash_able_group_id_422}  parking_id=${parking_id_422}  
   ${unessential_params}  create dictionary  
...
get user wash able groups Success 
   ...
   ${unessential_params}  create dictionary  wash_area_id=${wash_area_id}  
...
get user wash able groups Fail With Wrong Params
   ...
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  
   ${unessential_params}  create dictionary  wash_area_id=${wash_area_id_422}  
...
get wash groups Success 
   ...
   ...              预期结果:输入正确参数,http响应码返回 200,返回的Json数据为 WashGroup 列表。
   [Tags]           Respcode:200
   ${essential_params}  create dictionary  status=${status}  wash_area_id=${wash_area_id}  parking_id=${parking_id}  
   ${unessential_params}  create dictionary  
...
get wash groups Fail With Wrong Params
   ...
   ...              预期结果:输入错误参数,http响应码返回 422,返回的Json数据为错误信息。
   [Tags]           Respcode:422
   ${essential_params}  create dictionary  status=${status_422}  wash_area_id=${wash_area_id_422}  parking_id=${parking_id_422}  
   ${unessential_params}  create dictionary  
...
*** Keywords ***
post user wash groups Success 200
   [Arguments]  &{kwargs}
   ${resp}=  post user wash groups  &{kwargs}
   expect status is 200  ${resp}  wxmp_wash_group/post_user_wash_groups_200.json
